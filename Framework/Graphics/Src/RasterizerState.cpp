...
{
	OMEGAASSERT(mRasterizerState == nullptr, "RasterizerState -- Rasterizer state not released!");
}
...
void RasterizerState::Initialize(CullMode cullMode, FillMode fillMode)
{
	...
	OMEGAASSERT(SUCCEEDED(hr), "RasterizerState -- Failed to create rasterizer state.");
}
