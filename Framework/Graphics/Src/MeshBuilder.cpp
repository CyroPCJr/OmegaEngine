...
MeshPX MeshBuilder::CreatePlanePX()
{
	float height = 1.f;
	float width = 1.f;
	MeshPX mMesh;
	for (float y = 0.0f; y < height; y+= 0.1f)
	{
		for (float x = 0.0f; x < width; x+= 0.1f)
		{
			mMesh.vertices.emplace_back(VertexPX{ Vector3{x,y,0.0f} , (1.0f / x), (1.0f / y) });
		}
	}
